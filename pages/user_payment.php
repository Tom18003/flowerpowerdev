...
$cart_table = mysqli_query($dbconn,"SELECT sum(total) FROM order_details WHERE user_id='$user_id' AND order_id=''") or die(mysqli_error());
       $cart_count = mysqli_num_rows($cart_table);
        ...
        while ($cart_row = mysqli_fetch_array($cart_table)) {
           ...
           echo '********* uw bestel nummer: '.$track_num.' | ';  
           echo 'Totaal: €'.$total.' | ';
           echo 'Addres: '.$ship_add.' *********';
...
?>
        ...
        <hr color="orange"> 
        <br><br>
        <h3>Betaal contant bij levering</h3>
        <h3>uw bestelling zal geleverd worden binnen 3 tot 5 werkdagen.</h3><br>
        <h5>Flower Power</h5>
